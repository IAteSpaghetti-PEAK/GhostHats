...
This file is the developer side of things. The mod page copy — what it does, install, multiplayer —
lives in [thunderstore/README.md](thunderstore/README.md), which is what ships in the package.
...
`resources.assets` are a jumbled pool rather than contiguous per prefab, so don't bother going
that way — which rules out anchoring to a named head bone.

Instead, placement is measured. Both the character and the ghost carry the same face pieces: two
eye renderers and a mouth renderer. Those define a frame — origin at the eye midpoint, right along
the eye line, up from the mouth towards the eyes, forward from their cross product. The hat's
transform is read in the character's face frame and re-applied in the ghost's, scaled by the ratio
of the two eye spacings.
...
The widest-apart pair of eye renderers is used, so extra shadow/overlay renderers in either array
don't throw the frame off.
...
Diagnostics go through `LogDebug` instead of a verbose-logging setting: quiet by default, and
anyone chasing a problem can enable Debug in `BepInEx.cfg`. It logs the measured scale ratio and
resulting local position per ghost, which is what you want if placement ever looks off.
...
| `thunderstore/` | manifest, 256×256 icon, changelog, package README |
...
Builds `artifacts/GhostHats-<version>.zip` and refreshes `release-assets/` (committed, because the
GitHub runner can't build this — it needs PEAK's own DLLs). `thunderstore/manifest.json` is the
